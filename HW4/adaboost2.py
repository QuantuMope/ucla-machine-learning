...
class DecisionStump:
    ...
    def fit(self, X, D, y):
        ...
        for j in range(d):
            sorted_order = X[:, j].argsort()
            X_sorted = X[sorted_order]
            y_sorted = y[sorted_order]
            D_sorted = D[sorted_order]
            m_plus1 = X_sorted[-1, j] + 1
            Fp = D_sorted[y_sorted == 1].sum()
            Fm = D_sorted[y_sorted == -1].sum()
            if Fp < Fs:
                Fs = Fp
                self.theta = X_sorted[0, j] - 1
                self.j = j
                self.b = 1
            if Fm < Fs:
                Fs = Fm
                ...
                self.b = -1
            for i in range(m):
                Fp = Fp - y_sorted[i] * D_sorted[i]
                Fm = Fm + y_sorted[i] * D_sorted[i]
                xij = X_sorted[i, j]
                xi1j = X_sorted[i+1, j] if i != m-1 else m_plus1
                if Fp < Fs and xij != xi1j:
                    Fs = Fp
                    self.j = j
                    self.theta = 0.5 * (xij + xi1j)
                    self.b = 1
                if Fm < Fs and xij != xi1j:
                    Fs = Fm
                    self.j = j
                    self.theta = 0.5 * (xij + xi1j)
                    self.b = -1
...
class MyAbaboost:
    ...
    def fit(self, X_train, y_train, X_test, y_test, verbose=False):
        ...
        for i in range(self.n_estimators):
            ...
            # Implementation starts from here
            h_tx = ht.predict(X_train)
            epsilon_t = D[y_train != h_tx].sum()
            w = 0.5 * np.log(1 / epsilon_t - 1)
            num = D * np.exp(-w * y_train * h_tx)
            denom = num.sum()
            D = num / denom
            # Implementation ends from here
